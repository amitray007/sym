...
**Control tier — intentional local state.** An operator TUI (Ink/React) and
`sym` CLI let the owner manage MCP connectors and their OAuth tokens, which
...
| `apps/agent`             | `@sym/agent`         | The runnable server — mounts all the above and adds routing, the Pi loop, MCP connector pool, operator TUI/CLI, and config.                                                                                                                                             |
...
| `mcp/`                 | MCP connector pool. `pool.ts` — module-level connection pool, `McpDispatcher`; `reconcile.ts` — live hot-reload without restart; `introspect.ts` — per-connector tool lists + test endpoint; `store.ts` — AES-256-GCM SQLite credential store; `inject.ts` — transport builder (stdio / HTTP+SSE); `config.ts` — `ConnectorConfig` type; `source.ts` — config file reader; `composite.ts` — `CompositeDispatcher` (builtins + MCP); `oauth-registry.ts` — pending OAuth flow tracking; `providers/` — OAuth + static-token provider impls; `cli-config.ts` — `cli` section reader. |
| `cli/`                 | `sym` operator CLI. `index.ts` — commander entry + isTTY guards; `admin-client.ts` — HTTP client for `/admin/*`; `config-store.ts` — local JSON config; `secrets.ts` — stdin-only secret set.                                                                                                                                                                                                                                                                                                                                                                                      |
| `tui/`                 | Ink/React operator dashboard. `index.tsx` — launch + `NO_COLOR`/isTTY hardening; `app.tsx` — screen router; `screens/Dashboard.tsx`, `screens/DetailScreen.tsx`, `screens/BuilderScreen.tsx`, `screens/SecretsManager.tsx`; `ui/` — shared components and theme.                                                                                                                                                                                                                                                                                                                   |
...
The operator TUI/CLI and the AES-256-GCM SQLite credential store are

...
| `packages/cursor-runtime` | `@sym/cursor-runtime` | The Cursor cloud-agent runtime (opt-in second execution tier): `CursorCloudClient` over `@cursor/sdk` cloud mode, the control-tier SQLite run store (`store.ts`, intent-row lifecycle), the poll reconciler (`reconciler.ts`), and the repo allowlist. Dispatches a coding task to an isolated Cursor cloud VM that opens a PR. Depends only on `@cursor/sdk` + `zod`.                                                                                    |
...
```
                 @sym/contracts
        ↑              ↑               ↑
  @sym/kernel   @sym/mcp-runtime   @sym/adapter-slack      @sym/cursor-runtime
        └──────────────┴───────────────┴──────────────────────────┘
                       ↑
                  @sym/agent   (depends on all four)
```

`packages/*` never import from `apps/*`. `@sym/contracts` imports nothing in
`@sym/*`. `@sym/cursor-runtime` depends on no `@sym/*` package (its types are
package-local, like a self-contained SDK adapter). Circular dependencies are a
CI error.
...
| `tools/`               | One file per tool family. `registry.ts` owns `ALL_BUILTIN_DESCRIPTORS` and the `Map`-based dispatch table. `_helpers.ts` has shared error/arg helpers. Families: `slack-read`, `slack-write`, `slack-search`, `slack-users`, `web`, `planning`, `presentation`, `time`, and (opt-in) `cloud-agent` (`dispatch_cloud_agent`, registered per-turn only when `CURSOR_API_KEY` is set; `destructiveHint` → owner confirmation). |
...
`SYM_PERSONAS_DIR`) for exactly this reason. The opt-in cloud-agent run store
(`SYM_CLOUD_DB_PATH`, `cloud.db`) is the same kind of control-tier state and
must be pinned to `/data` too; if it can't be opened the feature fails open
(logs and disables itself) rather than crashing the agent.

**Cloud-agent tier (opt-in).** When `CURSOR_API_KEY` is set, Sym gains a third
capability beyond the two tiers above: the conversational loop can call
`dispatch_cloud_agent` to hand a coding task to an isolated Cursor cloud VM
(via `@sym/cursor-runtime`), which clones a repo, works, and opens a PR. A
boot-time poll reconciler tracks the run in the control-tier store and posts the
PR back into the originating thread. This does not change I-1: the run store
holds routing data only, and a single reconciler per deployment is assumed
(Sym is one deployable). With the key unset, none of this constructs — zero
behavior change.

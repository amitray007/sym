...
export async function createMcpConfig(formData: FormData): Promise<ActionResult> {
  ...
  // TODO(v2): capture envJson / oauthConfigJson when secrets management is built.
  ...
  if (await isSlugTaken(ctx.db, ctx.workspaceId, slug)) {
    return { ok: false, error: `A server with slug "${slug}" already exists in this workspace.` };
  }
...
export async function updateMcpConfig(
  ...
  if (!existing) return { ok: false, error: 'Server not found.' };
  ...
  await ctx.db.db
    .update(mcpConfigs)
    .set({ name, slug, url, updatedAt: new Date(), updatedByAdminId: adminId })
...
export async function deleteMcpConfig(id: string): Promise<ActionResult> {
  if (!(await requireAdmin())) return { ok: false, error: 'Forbidden.' };
  const ctx = await resolveContext();
  if (!ctx?.db) return { ok: false, error: 'Workspace not installed yet.' };
...
export async function setMcpConfigEnabled(id: string, enabled: boolean): Promise<ActionResult> {
  if (!(await requireAdmin())) return { ok: false, error: 'Forbidden.' };
  const ctx = await resolveContext();
  if (!ctx?.db) return { ok: false, error: 'Workspace not installed yet.' };

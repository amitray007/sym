/**
 * mcp/pool.ts — Module-level connection pool and McpDispatcher.
 *
...
/** Set the active configs — called by initMcpPool and reconcileConnectors. */
export function setActiveConfigs(configs: ConnectorConfig[]): void {
  activeConfigs = configs;
}

/** Stable structural equality for two connector configs (plain JSON shapes). */
export function configsEqual(a: ConnectorConfig, b: ConnectorConfig): boolean {
 ...
 */
export async function connectServer(config: ConnectorConfig): Promise<PoolEntry> {
  ...
  try {
    ...
    const mappedTools: ToolDescriptor[] = rawTools.flatMap((rawTool) => {
    ...
    // Z08-01: enforce tools.allow — filter to the declared allowlist when present.
    const tools: ToolDescriptor[] =
      config.tools?.allow !== undefined && config.tools.allow.length > 0
        ? mappedTools.filter((t) => {
            const localName = stripPrefix(t.name, config.name);
            return localName !== null && config.tools!.allow!.includes(localName);
          })
        : mappedTools;
 ...
 */
export async function ensureEntry(config: ConnectorConfig): Promise<PoolEntry> {

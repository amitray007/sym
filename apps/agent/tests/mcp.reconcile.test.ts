...
describe('reconcileConnectors — live pool reload', () => {
  ...
  it('adds a connector from an empty pool (status: connected) and serves its tools', async () => {
  ...
  it('leaves an unchanged connector untouched (status: unchanged, no reconnect)', async () => {
  ...
  it('reconnects when the config changes (status: reconnected)', async () => {
    await reconcileConnectors([echoConfig()]);
  ...
  it('validate-then-swap: a failed edit keeps the previous healthy connection serving', async () => {
  ...
  it('records a hard failure (status: failed) when nothing was serving the name before', async () => {
  ...
  it('removes a connector dropped from the config (status: removed) and stops serving it', async () => {
    await reconcileConnectors([echoConfig()]);
    const result = await reconcileConnectors([]);
    const echo = result.connectors.find((c) => c.name === 'echo');

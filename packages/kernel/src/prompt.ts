...
export function buildSystemPrompt(): string {
  return [
    ...
    "- For multi-step asks — anything needing 2+ distinct outcomes the owner cares about (research + summarize + reminder, find X and compare Y, draft + post + react) — call `set_plan` EXACTLY ONCE at the start with 2–6 short outcome-shaped items. Phrase each as what the owner asked for, NOT the tool you'll use: 'Find the latest incident' ✓, 'Call search_messages' ✗.",
    '- Then for each item: call `update_task` with `status: in_progress` when you start, `status: complete` when done. Use `status: blocked` with a `note` ONLY when you genuinely need owner input to proceed (missing link, ambiguous target, confirmation needed).',
    "- SKIP `set_plan` for single-outcome asks ('what time is it?', 'who did I DM yesterday?', 'summarize this thread', 'remind me at 5pm'). The card auto-renders tool steps in that case — no ceremony needed.",
    "- Once `set_plan` is called, it is locked for the turn. Don't try to call it again with a revised plan; if scope changes, just complete or block what you can and explain in the reply.",
    '',
    '## Quality bar',

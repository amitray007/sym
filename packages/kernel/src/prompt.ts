...
export function buildSystemPrompt(): string {
  return [
    ...
    '- You have three kinds of capability: built-in Slack + utility tools (always loaded); MCP CONNECTORS reached on demand via `find_tools` then `call_tool`; and command-line tools via `run_cli` (an argv array — no shell). The system context each turn lists the connectors and CLIs currently available.',
    ...
    '- When the owner asks what you can do, which integrations/connectors you have, whether something is connected, or how healthy it is: FIRST introspect, THEN answer from the result. Introspect with `run_cli` `["sym","status"]` (connector health + tool counts) or `["sym","tools"]` (every tool, with descriptions), and/or `find_tools "<what you want>"`. Add `"--json"` to a sym command for exact structured data. Treat that fresh output as the source of truth — never the cached list.',
    '- To USE a connector capability: `find_tools` for the exact tool name + input schema, then `call_tool`. To use a CLI: `run_cli`; if you don’t know its subcommands, run `["<cli>","--help"]` first, then the real command. Before telling the owner you cannot do something, introspect — the capability may already be there.',
    '',

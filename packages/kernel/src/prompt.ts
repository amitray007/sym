...
export function buildSystemPrompt(): string {
  return [
    ...
    '- To DO something in an external system (cloud, error tracker, repo, issue tracker, docs, …), follow this order:',
    '  1. ORIENT. Read the connectors + CLIs in the turn context. If unsure whether something exists or is healthy, introspect FIRST: `run_cli ["sym","status"]` (what is connected + healthy) or `["sym","tools"]` (every connector tool with a description). Append `"--json"` for exact data. Treat that fresh output as the source of truth.',
    '  2. If the capability is an MCP CONNECTOR: call `find_tools "<your goal>"` to get the exact tool name + input schema, then `call_tool` with arguments matching that schema.',
    '  3. If the capability is a CLI (it appears in the run_cli catalog — e.g. gcloud, sentry-cli, gh, jq): call `run_cli` with the argv, e.g. `["gcloud","projects","list"]`. If you do not know the subcommands or flags, run `["<cli>","--help"]` (or `["<cli>","<subcommand>","--help"]`) FIRST, then the real command. Credentials are already wired into the environment — never ask the owner for a token or key.',
    ...
    '- Before telling the owner you cannot do something in these domains, you MUST have introspected (`sym status`/`sym tools`/`find_tools`) — the capability may already be there.',
    '',

...
export function buildSystemPrompt(): string {
  return [
    ...
    '- A LINK IS NOT AUTOMATICALLY A `fetch_url`. When the owner hands you a URL, choose the reader by what the link points AT — don’t reflexively fetch it. A `slack.com/archives/…` permalink is a Slack message: pull the channel id (`C…`/`D…`) and the `p…` number (insert a dot 6 digits from the right — `p1700000000123456` → ts `1700000000.123456`; a threaded link carries `thread_ts=…` in the query) and use `read_thread` / `read_channel`; fetching it as HTML only hits a login wall. A link into a system you have a connector or CLI for (a GitHub PR/issue, a Notion/Linear page, a cloud console) → `find_tools` → that connector/CLI, for structured, authed data. `fetch_url` is the LAST resort, only for a generic public page (docs, article, blog) with no better reader — and it has no JS rendering, so auth-walled or app pages come back as junk.',

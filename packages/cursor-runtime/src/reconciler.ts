 ...
 *   - `running` past the max-run window → mark `error` (timed out) and deliver.
 ...
 *     window anchored at first observation (`pendingSince`), then delivers
 *     summary-only.
 ...
 *
 * Ticks are non-reentrant (a slow tick can't overlap the next and double-
 * deliver). Timers and the clock are injected so tests drive ticks
 * deterministically. Per-run failures are isolated — one bad run never stops
 * the loop. Logging is `console.info/warn/error` only (I-7).
 */

import { CursorClientError } from './client.js';
import { isTerminalStatus } from './types.js';

import type { CloudRunStore } from './store.js';
...
export interface CloudRunReconcilerOptions {
  ...
  prGraceMs?: number;
  /** Max age of a `running` run before it's failed as timed-out (ms). Default 6h. */
  maxRunMs?: number;
  now?: () => number;
...
const FINISHED_NO_PR_TEXT = 'finished (no PR)';
...
function shortStatusText(view: CloudRunView): string {
  switch (view.status) {
    case 'finished':
      return view.prUrl !== undefined ? 'opened PR' : FINISHED_NO_PR_TEXT;
    case 'error':
      return 'failed';
    case 'cancelled':
      return 'cancelled';
...
export class CloudRunReconciler {
  ...
  #timer: unknown;
  /** Reentrancy guard — a slow tick must not overlap the next (would double-deliver). */
  #running = false;
  ...
  constructor(options: CloudRunReconcilerOptions) {
    ...
    this.#maxRunMs = options.maxRunMs ?? 6 * 60 * 60 * 1000;
  ...
  /** One reconciliation pass. Public so tests can drive ticks without timers. Non-reentrant. */
  ...
  async #reconcileOne(row: CloudRunRecord): Promise<void> {
    ...
    // Anything not terminal/dispatching must be `running`; guard explicitly so an
    // unexpected status drains rather than silently looping forever.
    if (row.status !== 'running') {
      console.warn(`[cursor] unexpected status '${row.status}' for ${row.dispatchId}; failing`);
      this.#store.markStatus(row.dispatchId, 'error', { statusText: 'unexpected status' });
      await this.#deliver(row.dispatchId);
      return;
    }

    if (row.runId === undefined || row.agentId === undefined) {
      console.warn(`[cursor] running row ${row.dispatchId} missing run ids; failing`);
      this.#store.markStatus(row.dispatchId, 'error', { statusText: 'missing run ids' });
      await this.#deliver(row.dispatchId);
      return;
    }

    if (this.#now() - row.createdAt > this.#maxRunMs) {
      this.#store.markStatus(row.dispatchId, 'error', { statusText: 'timed out' });
      await this.#deliver(row.dispatchId);
      return;
    }

    await this.#pollRunning(row, row.agentId, row.runId);
  }

  /** Poll a confirmed running run and advance its stored state. */
  async #pollRunning(row: CloudRunRecord, agentId: string, runId: string): Promise<void> {
    let view: CloudRunView;
    try {
      view = await this.#client.getRun(agentId, runId);
    } catch (err) {
      if (err instanceof CursorClientError && !err.retryable) {
        this.#store.markStatus(row.dispatchId, 'error', { statusText: `failed (${err.code})` });
        await this.#deliver(row.dispatchId);
      } else {
        console.warn(`[cursor] poll failed for run ${runId}: ${errMsg(err)}`);
      }
      return;
    }
    ...
    if (view.status === 'finished' && view.pendingPr) {
      // Finished, but the PR URL hasn't surfaced. Wait, bounded by prGraceMs from
      // the FIRST finished-pending observation (not the run's start).
      if (row.pendingSince === undefined) {
        this.#store.markPendingSince(row.dispatchId, this.#now());
        return;
      }
      if (this.#now() - row.pendingSince > this.#prGraceMs) {
        this.#store.markStatus(row.dispatchId, 'finished', { statusText: FINISHED_NO_PR_TEXT });
        await this.#deliver(row.dispatchId);
      }
      return;
    }
